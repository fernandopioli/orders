...
from src.order.domain import Customer, Order
...
class TestOrder:
    ...
    def test_create_order_with_valid_data(self, valid_customer):
        ...
        result = Order.create(customer_id=str(customer_id), total=total)
    ...
    def test_update_order_total(self, valid_customer):
        order = Order.create(customer_id=str(valid_customer.id), total=100.0).value
